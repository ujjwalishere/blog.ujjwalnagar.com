---
Title: "My Geospatial Toolbox"
date: 2025-08-11
summary: "Collection of Tools that I use and found helpful in my work."
tech: ["GIS"]
repo: "https://github.com/ujjwalishere"
link: "https://example.com"
status: "active"
---
These are few collection of tools that I most often use and have used in my work. I will try to keep udating this list as I learn and use more tools. It is based on what I found useful in my research and projects, not a comprehensive list.


> I do not include proprietary and commercial stuff here, but there are a lot out there, some of them quite good.

# General Processing

| Tool                        | Use      |
| :---                        | :------: |
| [QGIS](https://qgis.org/) | Open source GIS Software which I most often use. |
| [Terrset LiberaGIS](https://www.clarku.edu/geospatial-analytics/terrset/) |
| [SNAP](https://step.esa.int/main/download/snap-download) | For working with Sentinel Data |
| [Google Earth Engine](https://earthengine.google.com/) | Free Cloud platoform for geospatial analysis |
| [Microsoft Planetary Computer](https://planetarycomputer.microsoft.com/)|  Another free cloud platform |
| [GDAL](https://gdal.org ) | Although included in almost every tool, but can be used in cmd |

# Data Sources

| Dataset | Description |
| :--- | :--- |
| [GADM](https://gadm.org/) | Boundaries for countries and subdivisions |
| [Indian Boundaries](https://onlinemaps.surveyofindia.gov.in/Digital_Product_Show.aspx) | Official boundaries from Survey of India (district, taluk, village level) |
| [GeoBoundaries](https://www.geoboundaries.org/) | Political and administrative boundaries globally with simplified files |
| [GAUL](https://data.apps.fao.org/catalog/organization/administrative-boundaries-fao) | UN FAO administrative boundaries from multiple agencies |
| [Natural Earth](https://www.naturalearthdata.com/) | Cartography-ready data endorsed by NACIS |
| [Bhoonidhi](https://bhoonidhi.nrsc.gov.in/) | National Geospatial Data Infrastructure portal of India |
| [Bhuvan](https://bhuvan.nrsc.gov.in/) | ISRO's geospatial platform with satellite imagery |
| [Copernicus](https://www.copernicus.eu/) | ESA satellite imagery and environmental monitoring |
| [EO Browser](https://www.sentinel-hub.com/explore/eobrowser/) | Sentinel Hub Earth observation data browser |
| [CHIRPS](https://www.chc.ucsb.edu/data/chirps) | Global rainfall data |
| [GHSL](https://human-settlement.emergency.copernicus.eu/) | Global Human Settlement Layer |
| [HydroSHEDS](https://www.hydrosheds.org/) | Hydrological data and maps |
| [AppEEARS](https://appeears.earthdatacloud.nasa.gov/) | NASA Earth data extraction tool |
| [NASA EarthData](https://search.earthdata.nasa.gov/search) | NASA Earth observation data hub |
| [SEDAC](https://www.earthdata.nasa.gov/centers/sedac-daac) | Socioeconomic and climate data |
| [NEO](https://neo.gsfc.nasa.gov/) | NASA Earth Observations visualization |
| [CIESIN Climate Data](https://ciesin.climate.columbia.edu/) | Columbia University climate data portal |

# Python and GeoProcessing

| Library                      | Use      | How to install  |
| :---                         | :------: | ----: |
| [Rasterio](https://github.com/rasterio/rasterio) | Reading and Manipulating rasters | `pip install rasterio` |


# Cartography

| Library                      | Use      | Link |
| :---                         | :------: | ----: |
| Color Brewer | Generating color palettee | https://colorbrewer2.org/ |





## General Tools

| Tool | Description |
| :--- | :--- |
| [MyGeoData Cloud](https://mygeodata.cloud/) | Online geospatial data conversion |
| [DIVA-GIS](https://diva-gis.org/) | Spatial data visualization |
| [Felt](https://felt.com/) | Collaborative mapping platform |
| [TileMill](https://tilemill-project.github.io/tilemill/) | Cartography tool |
| [GRASS GIS](https://grass.osgeo.org/) | Open source geospatial processing |
| [Mapnik](https://mapnik.org/) | Cartography rendering |
| [MapWindow](https://www.mapwindow.org/) | GIS software |
| [ZeeMaps](https://www.zeemaps.com/) | Online mapping tool |
| [DataBasin](https://databasin.org/) | Data aggregation and sharing |


## Courses & Resources

| Resource | Link |
| :--- | :--- |
| [Spatial Thoughts](https://spatialthoughts.com/) | |
| [NASA Panoply](https://www.giss.nasa.gov/tools/panoply/) | |
| [Equal Earth](https://equal-earth.com/) | |
| [NACIS](https://nacis.org/) | |
| [Wisconsin Cartography](https://geography.wisc.edu/cartography/) | |
| [Shaded Relief](https://www.shadedrelief.com/) | |
| [Kelso Cartography](https://kelsocartography.com/) | |
| [Red Geographics](https://www.redgeographics.com/) | |
| [Springer Cartographics](https://springercartographics.com/) | |
| [FREAC](https://freac.fsu.edu/) | |
| [Avenza](https://www.avenza.com/) | |
| [Stamen](https://stamen.com/) | |
| [Axis Maps](https://www.axismaps.com/) | |
| [MapTiler](https://www.maptiler.com/) | |


# Other Aggregated Sources

* [Columbia Library](https://guides.library.columbia.edu/geotools/home) - List of tools and datasets aggregated by Columbia Library
* [Brandeis Library](https://guides.library.brandeis.edu/c.php?g=990410&p=7164687) - Aggregated list from Brandeis Library